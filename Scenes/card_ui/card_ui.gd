...
@onready var stamina_icon = $StaminaIcon
@onready var card_art = $CardArt
@onready var card_banner = $CardBanner
@onready var drop_point_detector: Area2D = $DropPointDetector
@onready var card_state_machine: CardStateMachine = $CardStateMachine as CardStateMachine
@onready var targets: Array[Node] = []
@onready var original_index := self.get_index()
@onready var card_text = $CardDescription/MarginContainer/CardText
...
func _set_card(value: Card) -> void:
	...
	card_text.text = str(card.card_description)
...
func _set_playable(value: bool) -> void:
	...
	if not playable:
		...
		card_banner.modulate = Color(1, 1, 1, 0.5)
		stamina_icon.modulate = Color(1, 1, 1, 0.5)
	else:
