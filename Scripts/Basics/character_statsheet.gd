class_name CharacterStatsheet extends Resource

## A list of all properties that can receive bonuses.
const MODIFIABLE_STATS = [
	"max_health", "max_stamina", "stamina_rate", "lust", "charm", "constitution", "attack", "defense", "speed", "hit_chance", "evasion"
]
...
## Emitted whenver [member stamina] changes.
signal stamina_changed()
## Emiited whenever [member lust] changes.
signal lust_changed()
# Emitted when [member lust] reaches max
signal lust_filled()

## Elements and Statuses will be added Later!
...
@export var base_max_stamina := 6
@export var base_max_lust := 100
@export var base_stamina_rate := 3
